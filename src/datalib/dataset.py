...
from .utils import RandomShuffler, download_from_url, unicode_csv_reader
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, examples, fields, filter_pred=None, sort_key = None):
    ...
    def split(self, split_ratio=0.7, stratified=False, strata_field='label',
        ...
        splits = tuple(Dataset(d, self.fields, sort_key = self.sort_key)
    ...
    def stratify_split(self, strata_field):
        strata = stratify(self.examples, strata_field)
        splits = tuple(Dataset(d, self.fields, sort_key = self.sort_key) for d in strata)
        return splits
...
def merge_datasets(target, *srcs):
    for d in srcs:
