...
class Vocab(object):
    ...
    def __init__(self, counter, max_size=None, min_freq=1, unk_token='<unk>', specials=['<pad>'],
        ...
        self.stoi = defaultdict(_default_unk_index if unk_token is not None else None)
        # stoi is simply a reverse dict for itos
        self.stoi.update({tok: i for i, tok in enumerate(self.itos)})
        self.freqs = torch.tensor(self.freqs, dtype=torch.float)
